...
export default function VideoRecognizer(props) {
  let elRef = useRef(null);
  let pVideoRecognizer = useRef(null);
  let pCameraEnhancer = useRef(null);

  useEffect(()=>{
    (async() => {
      let cameraEnhancer = (pCameraEnhancer.current = pCameraEnhancer.current || await CameraEnhancer.createInstance());
      cameraEnhancer.setUIElement(elRef.current);
      ...
      let recognizer = (pVideoRecognizer.current = pVideoRecognizer.current || await LabelRecognizer.createInstance());
    ...
    return async () => {
      if (!pVideoRecognizer.current && pCameraEnhancer.current) {
        await pVideoRecognizer.current.destroyContext();
        console.log('VideoRecognizer Component Unmount');
      } else if(pVideoRecognizer.current) {
        await pVideoRecognizer.current.destroyContext();
        pCameraEnhancer.current.dispose();
        console.log('VideoRecognizer Component Unmount');
      }
    }
  },[])
